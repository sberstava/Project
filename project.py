...
import sqlite3
selected = 0 # Айди записи
# Создание бд, подключение
...
# Функция, изменяющее существующего сотрудника
def update_employee():
    global selected
    full_name = entry_full_name.get()
    phone_number = entry_phone_number.get()
...
label_full_name = tk.Label(window, text="ФИО сотрудника:")
...
label_email = tk.Label(window, text="Адрес эл.почты:")
...
button_update.grid(row=7, column=2, padx=5, pady=5)

button_delete = tk.Button(window, text="Удалить", command=delete_employee)
button_delete.grid(row=7, column=2, padx=5, pady=5)
button_delete.place(x=520, y=293)

button_re = tk.Button(window, text="Обновить список", command=refresh_employee_list)
button_re.grid(row=7, column=1, padx=5, pady=5)
button_re.place(x=300, y=401)
